...
if ! command -v screen &> /dev/null; then
    ...
    if ! command -v screen &> /dev/null; then
        ...
        sudo python audio-player.py --force-update --max-downloads=10 > audio.log 2>&1 &
...
screen -dmS audio-player bash -c "cd $BASE_DIR && sudo python audio-player.py --force-update --sync-interval=60 --max-downloads=10; exec bash"
